...
class SecretEditViewAccessTestCase(TestCase):
    """
    Secret creation/editing no longer gates on the requesting user having an
    active UserKey: the master key is resolved automatically (see
    ``netbox_secrets.utils.get_auto_master_key``), so users are never
    redirected to set up or activate a personal key just to reach the form.
    """
    ...
    def test_no_redirect_without_userkey(self):
    ...
    def test_no_redirect_with_inactive_userkey(self):
...
class SecretEditViewPostTestCase(TestCase):
    ...
    def setUp(self):
        ...
        patcher = mock.patch('netbox_secrets.views.utils.get_auto_master_key', return_value=self.master_key)
        self.mock_get_auto_master_key = patcher.start()
        self.addCleanup(patcher.stop)

    def _post_data(self, plaintext='secret', plaintext2='secret', extra=None):
        data = {
            'name': 'secret',
            'role': self.role.pk,
            'plaintext': plaintext,
    ...
    def test_post_no_master_key_available(self):
        ...
        self.assertTrue(any("No master key is available" in err for err in form_errors))
    ...
    def test_post_add_another(self):
        response = self.client.post(
            self._post_url(),
            data=self._post_data(extra={'_addanother': '1'}),
        )
        self.assertHttpStatus(response, 302)
